...
`GAME_VERSION` auf **1.9.0** angehoben (weitere sichtbare Änderung). Testabdeckung erweitert: 24/24 jsdom-Prüfungen grün (6 neue, u. a. explizite Prüfung auf „AGENT"/„CONTRACT" als einzelne Wörter, nicht nur als Phrase) plus – neu für dieses Projekt – ein echter visueller Check per Playwright (Headless-Chromium, lokal in der Sandbox gerendert, kein `file://`-Zugriff über die Chrome-Erweiterung nötig): Screenshot der Einladung und der Szenario-Seite erzeugt, von mir gegengelesen und Stephan zur Freigabe vorgelegt („passt"). Dieser Weg ist deutlich zuverlässiger als der bisherige rein datengetriebene jsdom-Test und sollte künftig Standard für optische Änderungen an diesem Projekt sein.
`public/index.html` erneut auf Stephans Mac geschrieben (Basis: v1.8.0 / Commit `2103a8a`, `git status` vorher sauber geprüft).
